            ...
            <?php foreach ($rows as $row) : ?>
              <tr>
                ...
                <!-- <td data-label="Update">
                  <img src='<?= ROOT ?>/assets/images/edit.svg' alt="edit" class="edit" id="<?= $row['code'] ?>">
                </td> -->
                <td data-label="Update">
      ...
      if (!empty($prows)) : ?>
        <?php foreach ($prows as $row) : ?>
          <form class='forum-form' method="POST">
            ...
            <input value="<?= $row['code'] ?>" type="text" id="code" name="code" placeholder="Code" class='inputs4' readonly>
            <br />

            <label for="Name">Name</label>
            <input value="<?= $row['name'] ?>" type="text" id="name" name="name" placeholder="Name" class='inputs4'>
            ...
            <input value="<?= $row['email'] ?>" type="text" id="email" name="email" placeholder="Email" class='inputs4'>
            <?php if (!empty($errors['email'])) : ?>
              <div class="error"><?= $errors['email'] ?></div>
            <?php endif; ?>
            <br />

            <input type="submit" name="add_new" value="Update Operator" class='primarybutton'>
          <?php endforeach; ?>
        <?php else :
        echo ("error");
      endif; ?>
          </form>
          <!-- <label for="add_new" class='error'><?= $data['errors']['add_new'] ?></label>
      <lable for="add_new" class='success'><?= $message ?></lable> -->
    </div>
