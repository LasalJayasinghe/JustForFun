...
<body id='loginpagebody'>
    <div class='container'>
        ...
        <div id="loginform">
        <div class='formtitle'>
                <a href="<?=ROOT?>/signup?usertype=customer">
            ...
            <form method="POST"> <!-- Front end bit to get login deets -->
                <input value="<?=set_value('vehicleno')?>" oninput="validateVehicleNo()" id="vehicleno" class='inputsignin' type="text" name="vehicleno" placeholder="Vehicle Number: XXX-nnnn" required1>
                <?php if(!empty($errors['vehicleno'])):?>
                    <div class="error"><?=$errors['vehicleno']?></div>
                <?php endif;?><br/>
                <div class="input-container">
                    <select id="vehicle-type" name='category' class="inputsignin dropdown">
                        ...
                        <option value="quad">Quadricycle</option>
                ...
                <br>
                <input value="<?=set_value('chassis_number')?>" class='inputsignin' type="text" name="chassis_number" placeholder="Vehicle Chassis Number" required1>
                <?php if(!empty($errors['chassis_number'])):?>
                    <div class="error"><?=$errors['chassis_number']?></div>
                <?php endif;?><br/>
                <input type="checkbox" name="terms" id="terms">&ensp; I agree to the terms and conditions

                <input type="hidden" name="owner_id" value="<?=$_SESSION['customerdetails']['nic']?>">

                <div id="buttoncontainer">
                    <button type="reset" class="secondarybutton">Cancel</button>
                    <button type="submit" id="submit-button" name='submit' class="primarybutton" value="Submit">Submit</button>
                </div>
                <div>
    ...
    <script>
    // This function validates the vehicleno input and updates the submit button accordingly.
    function validateVehicleNo() {
        // Get the vehicleno input.
        var vehicleno = document.getElementById('vehicleno').value;

        // Check if the input matches the required format.
        var isValid = /^[A-Za-z]{2,3}-\d{4}$/.test(vehicleno);
        ...
        if (isValid) {
            ...
            // If the input is not valid, disable the submit button and display an alert.
            // submitButton.disabled = true;
            // alert('Invalid vehicle number. The correct format is XXX-NNNN or XX-NNNN.');
        }
    }
    </script>
